<script>
  ...
  async function getSubscribers() {
    ...
    if (Array.isArray(subscribedChannels)) {
      subscribedChannels.forEach(async (channel) => {
        ...
        // get basics
        const res = await fetch(
          `https://youtol.de:3000/api/v1/mySubs/stats/basic?token=${token}&channelId=${channelId}`
        );
        const data = await res.json();
  ...
  async function getMyStats() {
    ...
    await Promise.all([
      fetch(`https://youtol.de:3000/api/v1/general/channelInfos/${token}`).then(
        async (res) => (data1 = await res.json())
      ),
      fetch(`https://youtol.de:3000/api/v1/myStats/channelStats/${token}`).then(
        async (res) => (data2 = await res.json())
      ),
    ]);
...
<main>
  <div class="column side">
    ...
    <button
      class="button"
      on:click={() => (myChannelData._visible = !myChannelData._visible)}
    >
    ...
    {#each subscribedChannelsData as channelData}
      <button
        class="button {channelData._visible ? 'focusKlasse' : ''}"
        on:click={() => (channelData._visible = !channelData._visible)}
      >
  ...
  <div class="column middle">
    ...
    <div
      class="container"
      style="display: {myChannelData._visible ? 'block' : 'none'}"
    >
      ...
      <div class="container-flex">
        <div class="grid-container">
          ...
          <div class="item">
            {myChannelData.subscriber}
            <div>Subscribers</div>
          </div>
          <div class="item">
            {myChannelData.videos}
            <div>Videos</div>
          </div>
        </div>
      </div>
  ...
  <div class="column middle">
    ...
    {#each subscribedChannelsData as channelData}
      <div
        class="container"
        style="display: {channelData._visible ? 'block' : 'none'}"
      >
        ...
        <div class="container-flex">
          <div class="grid-container">
            ...
            <div class="item">
              {channelData.subscriberCnt}
              <div>Subscribers</div>
            </div>
            <div class="item">
              {channelData.videoCnt}
              <div>Videos</div>
            </div>
          </div>
        </div>
